...
from model.logic import timer, get_ip, danger, flag
...
def main(bot, conn, client):
    ...
    # console_flow = Thread(target=console, args=(bot, conn))
    # bot_work.start()
    bot_logic.start()
    timer_danger.start()
    ip = get_ip()
    # console_flow.start()
    mailing(bot, f'Бот запущен на сервере у {ip}', [1011917065, 453207183, 809971387])
    # console_flow.join()
    start_bot(bot)
    bot_stop(bot, bot_logic, timer_danger)
    conn.close()


# def console(bot, conn):
#     while True:
